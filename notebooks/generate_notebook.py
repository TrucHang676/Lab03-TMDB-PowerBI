 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "else:\n",
    "    print(\"\\nTat ca cac cot bat buoc deu ton tai.\")\n",
    "\n",
    "display(df.head())"
   ]
  },
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "### 3.6. Bổ sung dữ liệu bằng TMDB API\n",
    "\n",
    "API enrichment là bước bổ sung dữ liệu có truy vết, giúp làm giàu thông tin cho các phim thực sự cần thiết mà không thay thế hay làm ảnh hưởng đến dữ liệu gốc. Quy trình tuân thủ các nguyên tắc sau:\n",
    "- **Bổ sung dữ liệu có truy vết**: Mọi thay đổi đều được ghi nhận qua các trường trạng thái (`DaBoSungTuAPI`, `NguonBoSung`, `ThoiDiemBoSung`).\n",
    "- **Dữ liệu API không thay thế dữ liệu gốc**: Giữ nguyên các cột dữ liệu gốc (`TenPhim`, `TenGoc`, `ThoiLuong`, `NganSach`, `DoanhThu`, `MaIMDb`). Các trường dữ liệu từ API được lưu vào các cột riêng biệt có hậu tố `API` hoặc `TiengAnh` (`TenPhimTiengAnh`, `TomTatNoiDungTiengAnh`, `ThoiLuongAPI`, `NganSachAPI`, `DoanhThuAPI`, `MaIMDbAPI`).\n",
    "- **Các cột dùng cho phân tích được đặt tên riêng để tránh nhầm lẫn**: Tạo thêm các cột phân tích riêng biệt (`TenPhimHienThiCuoi`, `ThoiLuongPhanTich`, `NganSachPhanTich`, `DoanhThuPhanTich`) kèm nguồn xác định (`NguonThoiLuong`, `NguonNganSach`, `NguonDoanhThu`) để phục vụ trực quan hóa, tránh nhầm lẫn với các cột gốc."
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "        print(f\"Loi khi doc cache file: {e}\")\n",
    "        api_cache = {}\n",
    "else:\n",
    "    api_cache = {}\n",
    "\n",
    ...
    "print(f\"Tong so phim can bo sung thong tin: {total_needed}\")\n",
    "\n",
    ...
    "    print(\"Phuong thuc xac thuc dang dung: Bearer token\")\n",
    "elif api_key:\n",
    "    auth_method = \"API key\"\n",
    "    print(\"Phuong thuc xac thuc dang dung: API key\")\n",
    "else:\n",
    "    auth_method = \"Khong co xac thuc, chi dung cache\"\n",
    "    print(\"Canh bao: Khong tim thay TMDB_BEARER_TOKEN hay TMDB_API_KEY. Bo qua buoc goi API moi, chi dung du lieu san co trong cache.\")\n",
    "    \n",
    "# 4. Thực hiện gọi API\n",
    "cache_updated = False\n",
    "success_count = 0\n",
    ...
    "                \n",
    "            try:\n",
    "                time.sleep(0.2) # Delay nhe 200ms\n",
    ...
    "                    # 404 Not Found, cache lai de tranh goi lap lai\n",
    "                    api_cache[movie_id] = {\n",
    "                        \"status\": \"not_found\",\n",
    "                        \"data\": None,\n",
    "                        \"timestamp\": datetime.datetime.now().isoformat()\n",
    ...
    "                    error_count += 1\n",
    "                else:\n",
    "                    print(f\"Loi API voi phim ID {movie_id}: HTTP {response.status_code}\")\n",
    "                    error_count += 1\n",
    "            except Exception as e:\n",
    "                print(f\"Loi ket noi khi goi API cho phim ID {movie_id}: {e}\")\n",
    "                error_count += 1\n",
    "        else:\n",
    "            # Khong co token/key va khong co trong cache -> Khong thuc hien ghi nhan API\n",
    "            pass\n",
    "\n",
    ...
    "        print(\"Da cap nhat cache file moi.\")\n",
    "    except Exception as e:\n",
    "        print(f\"Loi khi ghi cache file: {e}\")\n",
    "\n",
    ...
    "\n",
    "filtered_df[\"NguonThoiLuong\"] = \"Khong co\"\n",
    "filtered_df.loc[filtered_df[\"ThoiLuongAPI\"].notna(), \"NguonThoiLuong\"] = \"API\"\n",
    "filtered_df.loc[filtered_df[\"ThoiLuong\"].notna(), \"NguonThoiLuong\"] = \"Goc\"\n",
    "\n",
    ...
    "\n",
    "filtered_df[\"NguonNganSach\"] = \"Khong co\"\n",
    "filtered_df.loc[filtered_df[\"NganSachAPI\"].notna(), \"NguonNganSach\"] = \"API\"\n",
    "filtered_df.loc[filtered_df[\"NganSach\"].notna(), \"NguonNganSach\"] = \"Goc\"\n",
    "\n",
    ...
    "\n",
    "filtered_df[\"NguonDoanhThu\"] = \"Khong co\"\n",
    "filtered_df.loc[filtered_df[\"DoanhThuAPI\"].notna(), \"NguonDoanhThu\"] = \"API\"\n",
    "filtered_df.loc[filtered_df[\"DoanhThu\"].notna(), \"NguonDoanhThu\"] = \"Goc\"\n",
    "\n",
    ...
    "\n",
    "print(\"Thong ke lam giau du lieu bang TMDB API:\")\n",
    "print(f\"  - So phim can goi API: {total_needed}\")\n",
    "print(f\"  - So phim lay tu cache: {cache_hits}\")\n",
    "print(f\"  - So phim goi API thanh cong: {success_count}\")\n",
    "print(f\"  - So phim loi API: {error_count}\")\n",
    "print(f\"  - So phim bo sung duoc TenPhimTiengAnh: {has_title_en}\")\n",
    "print(f\"  - So phim bo sung duoc TomTatNoiDungTiengAnh: {has_overview_en}\")\n",
    "print(f\"  - So phim bo sung duoc ThoiLuongAPI: {has_runtime}\")\n",
    "print(f\"  - So phim bo sung duoc DoanhThuAPI/NganSachAPI: {has_budget_rev}\")\n",
    "print(f\"  - Da xuat file enriched thanh cong tai: {OUTPUT_ENRICHED_PATH}\")"
   ]
  },
  {
   "cell_type": "markdown",
